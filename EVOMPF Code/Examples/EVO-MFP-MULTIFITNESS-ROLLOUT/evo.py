...
from EasyChemML.JobSystem.Module.ModelJob import ModelTrainEvalJob
...
for fingerprint_file in fingerprints_files:
    ...
    env = Environment(WORKING_path_addRelativ=tail + '_Output')
    ...
    catboost_r_model_BIG = Config(CatBoost_r, {'verbose': 0, 'thread_count': thread_count, 'depth': 12, 'iterations': 50000,
                                         'allow_writing_files': False})
    catboost_r_model_LIPO = Config(CatBoost_r, {'verbose': 0, 'thread_count': thread_count, 'depth': 10, 'iterations': 10000,
                                          'allow_writing_files': False})
    catboost_c_model = (CatBoost_c, {'verbose': 0, 'thread_count': thread_count, 'allow_writing_files': False})
    job: ModelTrainEvalJob = job_factory.create_ModelTrainEvalJob('lipoJOB', dataset_lip, catboost_r_model_LIPO,
                                                                  metricStack_r,
                                                                  dataset_lip.get_Split().
                                                                  get_outer_split(0))
