...
from EasyChemML.Encoder.Abstract_Encoder import Abstract_Encoder
...
from EasyChemML.Utilities.DataUtilities.BatchTable import Batch_Access, BatchTable
...
class EvoFP(Abstract_Encoder):
    ...
    def train(self, n_jobs: int, populationSize: int, newgen_rate: float,
        ...
        """
            ...
            Args:
                ...
                newgen_rate (float): Rate at which new generations are created.
                gen_recombinationrate (float): Rate of genetic recombination.
                newpop_perStep (float): The percentage of new fingerprints generated per generation.
                usebestforkids (float): This parameter specifies the percentage of the best fingerprints to be considered for inheritance.
                keepbest_perStep (float): Percentage of keeping the best individuals in each generation.
                aging_rate (float): This parameter ensures that the genes age and are thus more likely to be replaced by new ones with increasing generation.
                fp_size (int): The number of genes, i.e. SMARTS queries, per fingerprint
                evo_steps (int): Number of evolutionary steps.
                fitfunc (Abstract_Fitnessfunction): A Object with type Abstract_Fitnessfunction to evaluate individuals.
                feature_typ (FeatureTyp): Type of features used in the training.
                fitfunc_worker (int): Number of workers for fitness function evaluation.
                environment (Environment): Training environment settings.
                train_data_path (str, optional): Path for saving intermediate trainingdata. Defaults to 'CV_EVOFP'.

            Returns:
                SMART_Fingerprint

            This function trains a population of individuals using an evolutionary algorithm. It evolves the population over a specified number of steps (evo_steps) with various genetic operators and selection strategies based on the provided parameters. The fitness of individuals is evaluated using the given fitness function (fitfunc).
        """
    ...
    def convert(self, smart_fp: SMART_Fingerprint, feature_typ:FeatureTyp, datatable: BatchTable, columns: List[str], n_jobs: int):
        ...
        Returns:
            converted Dataset
        """
        iterator: Batch_Access = iter(datatable)
        batch: np.ndarray
    ...
    def __createFeaturesubset(self, inputMols, SMART_FP: SMART_Fingerprint, bitfeature, columns: List[str], n_jobs):
        ...
        iterator: Batch_Access = iter(inputMols)
        batch: np.ndarray
